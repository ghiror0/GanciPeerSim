...
/////////////////////////////////////////////////////////////////////////
	...
	private int selectCost(int size) {
		int best = -1;
		double actualBest = Long.MAX_VALUE;
		double temp;
		PathArrayNode path;
		for (int i = 0; i < pathList.size(); i++) { // per ogni path presente
			path = (PathArrayNode) pathList.get(i);
	...
	private int selectExec(int size) {
		int best = -1;
		long actualBest = Long.MAX_VALUE;
		long temp;
		PathArrayNode path;
		for (int i = 0; i < pathList.size(); i++) { // per ogni path presente
			path = (PathArrayNode) pathList.get(i);
	...
	public boolean checkFind() {
		for (Path path : pathList) {
			if (path.allNodeUp() && path.checkFind(this)) {
				return true;
			}
		}
		return false;
	...
	public void clearUnused() {
		...
		for (int i = size-1 ; i >= 0; i--) { 
			path =  pathList.get(i);
			if (path.checkRemove(this) || !path.allNodeUp()) {
				pathList.remove(i);
			}
		}
