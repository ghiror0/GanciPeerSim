...
public class MyProtocol implements CDProtocol {
	...
	public boolean addConcludedQuery(Query newQuery) {
		...
		concludedQuery.add(newQuery);		//TODO necessario getCopy()? sembra di no perche il messaggio di risposta ha una copia tutta sua
		return true;
	}
	...
	public void checkResponse() {
		for (Query query : concludedQuery) {
			if (CommonState.getTime() - query.getStartingTime() == maxWait) {
				//System.out.println("Tempo corrente: " + CommonState.getTime() + "\ttempo inizio: " + query.getStartingTime());
	...
	public void useCache(Query query) { 
		...
		Path cPath;
		for (int i = cachePath.size()-1; i>=0 ;i--) {
