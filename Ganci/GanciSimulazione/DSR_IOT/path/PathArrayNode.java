...
public class PathArrayNode implements Path{
	...
	public long getExecTime(int start, int end) {
		...
		for (NodeSingle node: nodes.subList(start, end+1)) {
			/*if(!checkNode(node.getId())) { //Non più necessario, faccio controllo prima di chiamarlo
...
////////////////////////////////////////////////////////////////////////////////////MODIFICHE EFFETTUATE PER METTERE NODEINFO////////////////////////////////
	...
	public boolean isRedundant2(Path Path) {
		...
		if (newPath.nodes.isEmpty() || (newPath.nodes.size()==1 && newPath.nodes.get(0).getId() == -1 )) { // Se non ci sono nodi, il path è considerato sempre ridondante
		...
		if (idIndex < 0) { // Se il primo nodo non è presente nel path, il nuovo path non può essere
							// ridondante
			if (verbose) System.out.println("non c'è primo nodo");
			return false;
		}
...
*	
	...
	private void getComposedSolutionPath2(QuerySingle query, NodeSingle node, PathArrayNode cPath) {
		...
		//if (checkParam(qOutput) >= 0)  return;// se l'output gia è presente, non cerco nulla
